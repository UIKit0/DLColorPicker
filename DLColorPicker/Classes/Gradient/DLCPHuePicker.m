...
- (void)setBrightness:(CGFloat)brightness {
	self.zValue = 1.0 - brightness;
}

- (CGFloat)brightness {
	return 1.0 - self.zValue;
}
...
- (void)setupWithXValue:(CGFloat)xValue yValue:(CGFloat)yValue zValue:(CGFloat)zValue {
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		UIImage *image = DLCPRenderHueImageWithWidth(yValue, zValue, 32);
...
- (void)respondToXValue:(CGFloat)xValue yValue:(CGFloat)yValue zValue:(CGFloat)zValue {
	...
	self.indicatorLayer.backgroundColor = [UIColor colorWithHue:xValue saturation:yValue brightness:zValue alpha:1.0].CGColor;
